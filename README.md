...
- ✅ Scope is the **Qwen 3.8 generation**, because all three SKUs are **single-tier** on the official tariff:

  | Key | Input | Output | Implicit cache | Context / out |
  |---|---|---|---|---|
  | `dashscope/qwen3.8-flash` | $0.15 | $0.47 | **$0.016** | 991,808 / 131,072 |
  | `dashscope/qwen3.8-max` | $2 | $6 | $0.25 | 991,808 / 131,072 |
  | `dashscope/qwen3.8-2.4t-a95b` | $2 | $6 | $0.25 | 991,808 / 131,072 |
...
- ❌ Exclude the **3.7 and older generations for now**: `qwen3.7-plus`, `qwen3.7-flash` and `qwen3.6-max-preview` are **tiered by request input size** (阶梯计价), which this catalogue's flat `input_cost_per_token` schema cannot express — LiteLLM models that with a `tiered_pricing` array that `should_exclude_due_to_price` does not read, which is also why upstream's `dashscope/qwen-flash` never reaches our export despite being priced. `qwen3.7-max` is single-tier but currently runs a 50%-off promotion with no published end date, so it needs the list-vs-effective decision made first
...
- Exported total **144 → 147** (+3); no existing entries changed.
