...
- ✅ **Currency: International USD, native.** Alibaba publishes two separate books — domestic 百炼 in CNY and International in USD — and upstream's `dashscope/*` prices are the USD ones (`dashscope/qwen3.8-max` is `$2 / $6` upstream while 百炼 lists `12 / 36` CNY, which is *not* 12÷7). We match that convention. **Do not** route these through `_cny_per_m_to_usd_per_token`
- ✅ `dashscope/qwen3.8-flash`: **$0.15 / $0.47** per M (input / output), single tier `0<Token≤1M` — unlike `qwen3.7-flash` and older, which are tiered by request input size
...
### v1.16.19 (2026-08-29)
- Add **DashScope (阿里云百炼 / Alibaba Cloud Model Studio)** as a provider with **Qwen3.8-Flash** (`dashscope/qwen3.8-flash`) — 1M context, multimodal, **$0.15 / $0.47** per M input / output, cache-hit **$0.03**. Source: [alibabacloud.com/help/en/model-studio/model-pricing](https://www.alibabacloud.com/help/en/model-studio/model-pricing), snapshot 2026-08-29.
  ...
  - **Cache-hit uses the implicit-cache rate.** Model Studio runs two cache modes: explicit hits at ~10% of standard input, implicit at ~20%. Implicit is automatic and cannot be disabled, so 20% (**$0.03/M**) is what an unconfigured caller pays — and being the higher of the two it never under-bills
  ...
  - ⚠️ `max_output_tokens` (32,768) is **inferred** from the upstream flash-tier sibling `dashscope/qwen-flash`; Alibaba's per-model spec page sits behind a console SPA that could not be read
...
- Exported total **144 → 145** (+1); no existing entries changed.
