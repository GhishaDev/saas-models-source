...
- ✅ **Two cache rates per SKU, two fields.** Every OpenAI image model publishes a *text* cached-input rate and a *separate, higher* **image** cached-input rate. `cache_read_input_token_cost` holds the text rate (matching upstream); **`cache_read_input_image_token_cost`** — a project-invented key with no upstream counterpart — holds the image rate. Added in v1.16.28 across all six `gpt-image-*` SKUs; before that the image rate was silently billed at the text rate, 20–50% light. ⚠️ Recording the rate is not the same as billing it: the internal LiteLLM fork's billing manager has to read the field before the shortfall actually closes
...
- ✅ Include: only keys listed in `ModelSyncRules.DASHSCOPE_ALLOWED_KEYS` (reverse whitelist, 6 SKUs today). Upstream carries 45 `dashscope/*` keys — Qwen plus third-party models resold through Model Studio — so the whitelist is what keeps the catalogue narrow
...
- ✅ Scope is the Qwen **3.8** and **3.7** generations. All prices are the **effective (post-discount)** ones — what a caller is billed today:
  ...
  | `qwen3.7-max` | — | $2.5 | $7.5 | $0.5 | 991,808 / 65,536 | ❌ |
...
- 🔍 **How to tell a promotion has lapsed — read the markup, not the number.** qwencloud renders a discounted price cell as three spans (`originalPrice`, `discountedPrice`, `discountTag`); an undiscounted cell is a bare price string. A lapsed promotion and a genuine price rise look identical in the rendered text, so the cell structure is the reliable signal. `qwen3.7-max` lapsed this way on 2026-09-04 (v1.16.24)
...
#### DeepSeek
- ✅ Include: only keys listed in `ModelSyncRules.DEEPSEEK_ALLOWED_KEYS` (reverse whitelist, **4 keys / 2 models** — the two SKUs on the official pricing page, plus `deepseek-v4-flash` and `deepseek-v4.1-flash` as aliases of `deepseek-flash`)
- ⚠️ **`deepseek-v4.1-flash` is a project alias, not a DeepSeek API name.** On the official page `DeepSeek-V4.1-Flash` appears **only in the MODEL VERSION row**; the callable names are exactly `deepseek-flash`, `deepseek-v4-flash`, `deepseek-v4-flash-vision-exp`, `deepseek-v4-pro`. Upstream carries no such key either — only third-party rehosts use the v4.1 spelling (`openrouter/deepseek/deepseek-v4.1-flash`, `fireworks_ai/.../deepseek-v4p1-flash`). It is carried for gateways that expose the version-style name to their own callers; **a request sent straight to api.deepseek.com under this name is expected to fail** — route it to `deepseek-flash`
- ✅ **Aliases are mirrored, never hand-written.** `deepseek-v4-flash` comes from upstream (byte-identical to `deepseek-flash` there); `deepseek-v4.1-flash` is produced by `apply_deepseek_alias`, which copies the canonical entry at build time via `DEEPSEEK_ALIAS_SOURCES`. Neither carries a literal price, so neither can drift from the canonical SKU — the failure mode that rotted six overlays in this file
...
- ✅ **List price, no promotion.** The row on [docs.z.ai](https://docs.z.ai/guides/overview/pricing) renders as three **bare** values, not the list+live price **pair** z.ai uses for a discount — so there is no revert to schedule, unlike GLM-5.3-Flash whose 50% promo lapsed on 2026-09-09 (v1.16.29). Same read-the-markup-not-the-number check as v1.16.24.
- **Pre-staged**: upstream carries no `zai/glm-5.3-flashx` (verified 2026-09-18). The `bigmodel/` twin holds **metadata only** — `apply_bigmodel_synth` copies the prices from the `zai/` sibling, so the FlashX tariff lives in exactly one place.
- ⚠️ **`max_output_tokens` 128000 is INFERRED, not published.** z.ai has no model page for FlashX (neither `guides/llm/glm-5.3-flashx` nor `guides/vlm/glm-5.3-flashx` exists), so the pricing row is the only authoritative data. 128000 is what every other GLM-5 SKU here carries, including GLM-5.3-Flash. Replace with a quote if z.ai publishes one. `max_input_tokens` 1000000 **is** published — 1M on both the z.ai table and the bigmodel 旗舰模型 table.
- 💱 The domestic bigmodel book quotes CNY (¥2 / ¥7 / ¥0.57 per M) and is **not** what is stored: `bigmodel/*` mirrors the z.ai **USD** book, as it has since v1.9.0. The two are separate tariffs, not one converted into the other.

### v1.16.37 (2026-09-18)
**No rate changed in this release.** Only key names and SKU coverage. Exported total **162 → 169**.

- 🔴 **Renamed the Seedream banded-price keys to match the gateway, and fixed the bare key's meaning.** v1.16.36 invented local names for the bands *and* put the **most expensive** band in `output_cost_per_image`. That combination caused a silent billing error: the dashboard pushes catalogue prices into the gateway's `litellm_params` as deployment-level custom pricing, so the gateway received three keys it does not define plus a flat override — which hits the branch where it **discards its own band table**. Result: 1024×1024 billed at **2×**, ≤1.5K layer decomposition at **4×**.

  | v1.16.36 key | → | Correct key | Value (Pro) |
  |---|---|---|---|
  | `output_cost_per_image` *(was >1.5K)* | → | `output_cost_per_image_above_1_5k` | ¥0.60 / 7 |
  | `output_cost_per_image_1_5k` | → | `output_cost_per_image_1_5k_and_below` | ¥0.30 / 7 |
  | `output_cost_per_image_layered` | → | `output_cost_per_image_layer_decomposition_above_1_5k` | ¥0.30 / 7 |
  | `output_cost_per_image_1_5k_layered` | → | `output_cost_per_image_layer_decomposition_1_5k_and_below` | ¥0.15 / 7 |
  | — | → | `output_cost_per_image` **(new meaning: fallback)** | ¥0.30 / 7 |

- ⚠️ **`output_cost_per_image` is the gateway's FALLBACK, not the headline price.** It is read only when no banded key matches — for single-price SKUs that makes it the only price; for Pro the gateway's own table puts the **≤1.5K** band there. Putting the >1.5K band in it was writing for a reader that does not exist. **Key names here are a contract, not cosmetics.**
- ➕ **Added the 7 missing SKUs** so the feed covers all 11 the gateway supports: `doubao-seedream-4-0` (+ `-250828`, `-20260415`), `4-5` (+ `-251128`), `5-0` (+ `-260128`). Previously an operator could not pick these from the Add Model dropdown at all — they had to type the model id by hand with no price pre-filled.
- 🎯 **Precision raised to exact division.** `_cny_per_image_to_usd_per_image()` no longer rounds to 4 sig figs. The gateway stores the unrounded quotient and is the side that actually bills; a rounded copy differs in the 5th digit and turns every future reconciliation into an argument. All five Pro band values and the bare key moved together.
- 🔗 `source` now carries `?lang=zh`, matching the gateway. `supported_endpoints` added. Lite's `input_cost_per_image: 0.0` **removed** — the gateway does not define that key for Lite, and a 0.0 would be pushed as an override of a field it does not have.
- 🩹 Two rule fixes the new SKUs required: `contains_date_pattern()` fires on the 8-digit `-20260415` stamp but not the 6-digit ones, so an `INCLUDE_PATTERN` now stops the generic date rule from second-guessing the whitelist; and the friendly-name formatter strips both stamp widths.
- ✅ **Verified field-by-field against `GhishaDev/litellm-internal`**: all 11 keys present on both sides, identical pricing-field names, values equal within 1e-6, matching `source` and `supported_endpoints`.
- 📎 The dashboard's compatibility shim (PR #767 / ADR-060) triggers on the presence of `output_cost_per_image_1_5k`. That key is gone here, so the shim becomes a no-op automatically — **no release ordering needed**, either side can ship first.

### v1.16.36 (2026-09-17)
- ➕ **Added the Seedream 5.0 image pair** — the **first non-video `volcengine/` SKUs**, extending that provider's scope beyond Seedance. `doubao-seedream-5-0-pro` and `doubao-seedream-5-0-lite`, each as a dated + alias pair (4 keys, same convention as Seedance). Exported total **158 → 162**. Source: the 图片生成模型 table on [volcengine.com/docs/82379/1544106](https://www.volcengine.com/docs/82379/1544106), read 2026-09-17.
- 💱 **Billed PER IMAGE in CNY (元/张)** — a different shape from every other SKU in the catalogue, which is per-token. New `_cny_per_image_to_usd_per_image()` helper; same fixed 7.0 policy FX, same 4-sig-fig rounding, and every stored value round-trips back to the published CNY (`round(val × 7, 2)`).
- **Seedream 5.0 Pro has a 2×2 output matrix** — scene × resolution:

  | | ≤261万像素 (≤1.5K) | >261万像素 (>1.5K) |
  |---|---|---|
  | 单图生成 single | ¥0.30 → `$0.04286` | **¥0.60 → `$0.08571`** |
  | 图层拆分 layered | ¥0.15 → `$0.02143` | ¥0.30 → `$0.04286` |

  `output_cost_per_image` carries the **ceiling** (¥0.60), so a consumer that ignores the variants over-bills visibly rather than under-billing silently — same never-under-bill rule as the DeepSeek peak tariff, the BytePlus list price and the DashScope tiered flats. The three cheaper cells are exposed as `output_cost_per_image_1_5k`, `_layered` and `_1_5k_layered`, mirroring Seedance's `output_cost_per_token_1080p` family.
- ⚠️ **`input_cost_per_image` is the marginal rate.** Volcengine gives the first input image free and charges ¥0.02 from the 2nd; the schema has no volume axis, so a single-image request is over-billed by ¥0.02 (~$0.003). Seedream 5.0 Lite's input is free outright (`0.0`).
- 🔴 **Latent bug fixed: `PRICE_FIELDS_BY_MODE["image_generation"]` listed only INPUT fields.** Seedream 5.0 Lite's *only* non-zero price is `output_cost_per_image` — free input, paid output — so it read as unpriced and would have been dropped even once whitelisted. **Third instance of this defect class**, after `tiered_pricing` (v1.16.19) and realtime `input_cost_per_second` (v1.16.26): a priced model vanishing because the vendor bills on an axis the tuple did not enumerate. Output fields are now listed.
- 🩹 **Fixed the Doubao friendly-name formatter, which assumed the family was always Seedance.** It hard-coded the literal `"Doubao-Seedance"` and used a regex that only stripped the `seedance` prefix — so the first non-Seedance volcengine SKU rendered as `Doubao-Seedance volcengine/doubao.seedream 5 0 Pro`: the prefix survived the failed substitution and was then split on `-`. The family is now **captured**, and the match doubles as a guard so an unrecognised key falls through to the generic formatter instead of producing that. All 32 existing Seedance / Dreamina names verified unchanged.
- ❌ **Excluded the Gemini Live pair** (`gemini-3.8-live`, `gemini-3.8-live-extended-thinking`, in both bare and `gemini/` forms). Both are `mode: realtime` with audio in and out — the modality the Google scope excludes — joining `gemini-3.1-flash-live-preview` and `gemini-3.5-live-translate-preview`. Like the transcribe pair in v1.16.22 they carry no `-preview` suffix, so nothing else kept them out; without these entries they would have entered alongside Seedream.

### v1.16.35 (2026-09-16)
- ➕ **Added `deepseek/deepseek-v4.1-flash`** as an alias of `deepseek-flash`, per request — **$0.30 / $1.20**, cache-hit **$0.006**, 1M context / 393,216 max output, vision. Exported total **157 → 158**.
- ⚠️ **It is a project alias, not a DeepSeek API name.** On [api-docs.deepseek.com](https://api-docs.deepseek.com/quick_start/pricing) `DeepSeek-V4.1-Flash` appears **only in the MODEL VERSION row**; the callable names are exactly `deepseek-flash`, `deepseek-v4-flash`, `deepseek-v4-flash-vision-exp` and `deepseek-v4-pro`. Upstream carries no `deepseek/deepseek-v4.1-flash` either — only third-party rehosts use the v4.1 spelling. **A request sent straight to api.deepseek.com under this name is expected to fail**; route it to `deepseek-flash`.
- ✅ **Mirrored, not hand-written.** New `DEEPSEEK_ALIAS_SOURCES` map + `apply_deepseek_alias` copy the canonical entry at build time, so the alias carries **no literal price** and cannot drift. Its `raw_data` is asserted equal to `deepseek-flash`'s; the two records differ only in `model_key` and `friendly_name`. Same mechanic as `NEWAPI_MIRROR_SOURCES`, minus the `litellm_provider` rewrite since the namespace is unchanged. An alias with a copied-out price is precisely the failure that rotted six overlays here (v1.16.18 / 20 / 23 / 24 / 29 / 30).
- 🔧 **Flattened the synth chain into `SYNTH_PIPELINE`.** It was a twelve-deep nested expression duplicated across `filter_all_models` and `get_filter_stats`, and its indentation had drifted out of step with its parentheses — inserting a step meant counting closing brackets in two places. Both call sites now iterate one shared ordered tuple, so they **cannot** diverge; the `stats.passed == len(filter_all_models)` invariant depends on exactly that.
  - **Proven behaviour-preserving**, not assumed: `filter_all_models` was run from the pre-refactor and post-refactor modules against the same upstream snapshot. Result — 0 removed, 0 changed, 1 added (the new alias).

### v1.16.34 (2026-09-16)
- ➕ **Restored `deepseek/deepseek-v4-flash` as a legacy alias** of `deepseek-flash`, per request. It was retired in v1.16.33; unlike the shut-down keys retired in v1.16.31–32 it still **resolves** — DeepSeek's footnote says the legacy name is *"still accepted… served by the DeepSeek-V4.1-Flash model and billed at the Flash price"* — so carrying it keeps existing callers working. Exported total **156 → 157**.
- ✅ **No price duplication risk.** Upstream holds `deepseek/deepseek-v4-flash` **byte-identical** to `deepseek/deepseek-flash` (verified field by field on 2026-09-16), so the alias tracks the V4.1 tariff automatically — **$0.30 / $1.20**, cache-hit **$0.006**, vision — and cannot drift from the canonical entry. In the export the two records differ only in `model_key` and `friendly_name`; their `raw_data` blocks are equal. No synth entry was added, which is the whole point: a hand-written copy is what would rot.
- ⚠️ **The alias's friendly name reads `DeepSeek-V4-Flash`, but the model behind it is V4.1-Flash.** The name derives from the key and the key is the legacy one. Prefer `deepseek-flash` for anything new.
- ❌ `deepseek/deepseek-v4-flash-vision-exp` stays **out**. It names an experimental vision variant that no longer exists as a distinct model, and V4.1-Flash has vision natively — so unlike `deepseek-v4-flash` that key is actively misleading, not merely redundant.

### v1.16.33 (2026-09-16)
- 🔴 **DeepSeek Flash was OVER-billing — the model was replaced and the price cut, and we had neither.** DeepSeek consolidated its Flash line into **DeepSeek-V4.1-Flash** and renamed the SKU to plain **`deepseek-flash`**, which the catalogue did not carry at all.

  | | Stored (V4-Flash) | Actual (`deepseek-flash`) | Delta |
  |---|---|---|---|
  | Input | $0.44 /M | **$0.30** /M | we charged **+47%** |
  | Output | $1.32 /M | **$1.20** /M | +10% |
  | Cache hit | $0.014 /M | **$0.006** /M | we charged **+133%** |

- ➕ Added **`deepseek/deepseek-flash`** — V4.1-Flash, **$0.30 / $1.20**, cache-hit **$0.006**, 1M context / 393,216 max output, **vision built in**, FIM in non-thinking mode. Upstream already carried it complete, so a whitelist entry was the only code needed.
- ❌ **Retired `deepseek/deepseek-v4-flash` and `deepseek/deepseek-v4-flash-vision-exp`.** ⚠️ Note the difference from v1.16.31–32: these are **not dead endpoints**. DeepSeek's footnote says the legacy names are *"still accepted, but the corresponding models have been retired, their requests are served by the DeepSeek-V4.1-Flash model and billed at the Flash price."* They are live aliases. They were dropped anyway because three keys pointing at one model is misleading, and `-vision-exp` in particular names an experimental vision variant that **no longer exists as a distinct model** — V4.1-Flash has vision natively.
- ✅ `deepseek/deepseek-v4-pro` verified **unchanged** ($1.32 / $3.96 / $0.044, no vision). DeepSeek also announced it is **continuing** V4-Pro service past 2026-09-14 with billing unchanged, so no action there.
- Exported total **157 → 156**.
- 🔎 **Third time an OpenAI key has entered the export with no decision made.** Regenerating surfaced `gpt-5.5-cyber` and `gpt-rosalind-research`, neither of which is on the official pricing page and neither of which has a model page on developers.openai.com. Both are now excluded — `gpt-5.5-cyber` is a superseded Daybreak model (the Cyber table lists only `gpt-5.6-sol` / `gpt-5.6-cyber`, and `gpt-5.4-cyber` is already scheduled for 2026-10-01 shutdown); `gpt-rosalind-research` belongs to the `*-deep-research` family already excluded. **OpenAI remains the only major provider with no reverse-whitelist** — its scope is enforced solely by exclusion patterns that a new key can simply fail to match. Flagged for the third time; fixing it properly means adding an `OPENAI_ALLOWED_KEYS` whitelist, which is a bigger change than any of these PRs.
...
- 🔴 **Every OpenAI `gpt-image-*` SKU was under-billing cached image tokens by 20–50%.** Each publishes *two* cache rates — one for text input, a higher one for image input — and LiteLLM's single `cache_read_input_token_cost` can only hold one. We hold the text rate (matching upstream), so cached image tokens were being billed at the text rate. This affects the whole family, not just the 2.5 pair flagged in v1.16.27:
  ...
  |---|---|---|---|
  | `gpt-image-1` | $1.25 /M | **$2.50** /M | **−50%** |
  | `gpt-image-1-mini` | $0.20 /M | **$0.25** /M | **−20%** |
  | `gpt-image-1.5` | $1.25 /M | **$2.00** /M | **−37.5%** |
  | `gpt-image-2` | $1.25 /M | **$2.00** /M ⚠️ | −37.5% |
  | `gpt-image-2.5-sunburst` | $1.25 /M | **$2.00** /M | −37.5% |
  | `gpt-image-2.5-flare` | $1.25 /M | **$2.00** /M | −37.5% |

- Adds **`cache_read_input_image_token_cost`** to all six. This is a **project-invented field** — LiteLLM's only image cost keys are `input_cost_per_image[_token]`, `output_cost_per_image[_token]` and `input_cost_per_image_above_128k_tokens`, none of them cached.
- **Why a new key rather than putting $2 into the existing one** (the call deferred in v1.16.27): reusing `cache_read_input_token_cost` would corrupt its meaning and over-bill cached *text* by 60%. A correctly-named new key has neither problem — existing consumers ignore it, so the change is non-breaking, and upstream can never overwrite it because upstream has no such key. The v1.16.27 reasoning ("don't invent a value locally") applies to *reusing an existing field*; it was never an argument against *adding* one.
- ⚠️ **Recording the rate is not yet billing it.** The internal LiteLLM fork's billing manager must learn to read this field before the shortfall closes. Until then this is documentation in a machine-readable shape.
- ⚠️ **`gpt-image-2`'s $2 is inferred, not quoted.** Its model page carries no pricing tables at all (checked twice on 2026-09-09). Basis: the `gpt-image-2.5-sunburst` and `-flare` pages both state *"Token rates match GPT Image 2"* and both publish $2 image cached input. Marked as inferred in the code; replace with a quote if OpenAI publishes one.
- ✅ **Gemini image models correctly have no such field.** [ai.google.dev](https://ai.google.dev/gemini-api/docs/pricing) prices Gemini image input as a single `$2.00 (text/image)` rate — one input tariff covering both modalities, so there is no second rate to store. OpenAI is dual-track, Google is single-track; the `None` on all six `gemini/*-image*` entries is the data source's shape, not a gap.
...
- ♻️ **Fixed by deleting the `DASHSCOPE_SYNTH_DATA` entry, not by editing its numbers.** The entry existed *solely* to overlay the effective price onto upstream's list price; with the promotion gone, effective **is** list, which is exactly what upstream carries. Everything else in the entry was already identical upstream (`max_input_tokens` 991,808, `max_output_tokens` 65,536, function calling, tool choice, reasoning, prompt caching), and the two negative flags were no-ops — upstream leaves `supports_vision` unset, and `supports_json_output` reads `supports_json_mode` with a `False` default. Fourth overlay retired on the same rule: **delete an overlay once upstream catches up.**
- 🔍 **New detection rule: read the markup, not the number.** A lapsed promotion and a genuine price rise are indistinguishable in rendered text. qwencloud's price cells are machine-readable — a discounted cell renders `originalPrice` + `discountedPrice` + `discountTag` spans, an undiscounted one is a bare string. The `qwen3.7-max` row now has three bare cells while `qwen3.7-plus` in the same table still carries the full triple, which is what confirmed the lapse rather than the price alone. Documented in both the DashScope rules section and `DASHSCOPE_SYNTH_DATA`.
- `qwen3.7-max` remains **text-only** (absent from both image-count tiers of the official vision docs); that rationale moved into the retirement note so it is not lost with the entry.
